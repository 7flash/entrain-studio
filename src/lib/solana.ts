...
type RpcTokenAccount = {
  account: { data: { parsed: { info: { mint?: string; tokenAmount: { uiAmount: number | null, uiAmountString?: string } } } } };
};

export async function getTokenBalance(publicKey: string, mint = TOKEN_MINT) {
  return await rpcMeasure.measure.assert({ label: 'Solana token balance', publicKey: publicKey.slice(0, 4), budget: 1200, timeout: 8000 }, async () => {
    ...
    let total = 0;
    for (const account of (payload.result?.value || []) as RpcTokenAccount[]) {
