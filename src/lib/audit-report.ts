...
import { analyzeSession, claimRisk, type ProtocolAnalysis } from '@/format/protocol-analyzer';
...
export function buildAuditReport(rows: AuditInputRow[], opts: { includeSignals?: boolean } = {}): AuditReport {
...
export function auditRow(row: AuditInputRow, opts: { includeSignals?: boolean } = {}): AuditRow {
  const session = sanitizeSession(row.session);
  const analysis = analyzeSession(session);
  const referenceId = row.lineageJson?.referenceId || row.lineageJson?.reference || '';
  const referenceMatch = compareToReference(session, referenceId);
  const claims = claimRisk(`${row.title || ''} ${row.summary || ''} ${row.description || ''} ${row.unlockNote || ''}`, { reviewed: !!row.copyReviewed });
  ...
  if (claims.risky) blockers.push(`claims:${claims.hits.join(',') || 'risky-copy'}`);
  if (referenceMatch && !referenceMatch.matches) blockers.push('reference:mismatch');
  if (row.patternHash && row.patternHash !== computedHash) warnings.push('stored-hash:stale');
  if (row.analysisJson && !roughEqual(row.analysisJson, analysis)) warnings.push('stored-analysis:stale');
  if (row.referenceMatchJson && !roughEqual(row.referenceMatchJson, referenceMatch)) warnings.push('stored-reference-match:stale');

  const verdict: AuditRow['verdict'] = blockers.length ? 'fail' : warnings.length ? 'warn' : 'ok';
  return {
    slug: row.slug,
    title: row.title,
    status: row.status || (row.isPublished ? 'published' : 'draft'),
    category: row.category || 'uncategorized',
    tier: row.tier || (Number(row.minTokens || 0) >= 100 ? 'collector' : Number(row.minTokens || 0) >= 10 ? 'pro' : Number(row.minTokens || 0) >= 1 ? 'holder' : 'free'),
    minTokens: Number(row.minTokens || 0),
    published: row.isPublished !== false && (row.status || 'published') === 'published',
    ...
    referenceMatch,
    claimRisk: claims,
    staleAnalysis: !!row.analysisJson && !roughEqual(row.analysisJson, analysis),
    staleReferenceMatch: !!row.referenceMatchJson && !roughEqual(row.referenceMatchJson, referenceMatch),
    signalMapSummary: {
