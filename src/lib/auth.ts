...
export async function verifyWallet(publicKey: string, signature: string, nonce: string) {
  return await authMeasure.measure.assert('Verify wallet', async () => {
    ...
    db.walletChallenges.update({ used: true, consumeToken }).where({ publicKey, nonce, used: false }).run();
    const consumed = db.walletChallenges.select().where({ publicKey, nonce, used: true, consumeToken }).first() as any;
