...
export function getSyncRoom(roomId: string) {
  const row = db.syncRooms.select().where({ roomId: normalizeRoomId(roomId) }).first() as any;
...
export function controlSyncRoom(
  ...
  return dbMeasure.measure.assert('Control sync room', () => {
    ...
    if (!hostKey || hostKey !== row.hostKey) throw new Error('Host key required');
    const now = Date.now();
    const patch: any = { updatedAt: now, expiresAt: now + ROOM_TTL_MS };
    if (action === 'start') {
      const offset = row.state === 'paused' ? Number(row.pausedOffsetSec || 0) : elapsedSec(row, now);
    ...
    db.syncRooms.update(patch).where({ roomId: normalizeRoomId(roomId) }).run();
    return toPublic(getSyncRoom(roomId) as any);
  });
}

export function heartbeatSyncRoom(roomId: string, body: {
  clientId?: string;
  label?: string;
  publicKey?: string;
  hostKey?: string;
  ...
  return dbMeasure.measure.assert('Heartbeat sync room presence', () => {
    ...
    const existing = db.syncRoomPresence.select().where({ roomId: row.roomId, clientId }).first() as any;
    const patch = {
      roomId: row.roomId,
      clientId,
      ...
      clientOffsetMs: Number.isFinite(Number(body.clientOffsetMs)) ? Number(body.clientOffsetMs) : undefined,
      rttMs: Number.isFinite(Number(body.rttMs)) ? Number(body.rttMs) : undefined,
      joinedAt: existing?.joinedAt || now,
      lastSeenAt: now,
    };
    if (existing) db.syncRoomPresence.update(patch).where({ roomId: row.roomId, clientId }).run();
...
export function leaveSyncRoom(roomId: string, clientId: string) {
  return dbMeasure.measure.assert('Leave sync room', () => {
    db.syncRoomPresence.delete().where({ roomId: normalizeRoomId(roomId), clientId }).run();
    const row = getSyncRoom(roomId) as any;
    return row ? toPublic(row) : null;
  });
}
...
export function toPublic(row: any): SyncRoomPublic {
  ...
  return {
    ...
    state: (row.state === 'playing' || row.state === 'paused') ? row.state : 'idle',
    serverNow: now,
...
function participantsForRoom(roomId: string, now = Date.now()): SyncRoomParticipant[] {
  ...
  return rows
    ...
    .map((r) => ({
      ...
      clientOffsetMs: r.clientOffsetMs === undefined ? undefined : Number(r.clientOffsetMs),
...
function prunePresence(roomId: string, now = Date.now()) {
  try {
    const rows = db.syncRoomPresence.select().where({ roomId }).all() as any[];
    for (const r of rows) if (Number(r.lastSeenAt || 0) < now - PRESENCE_TTL_MS) db.syncRoomPresence.delete().where({ roomId, clientId: r.clientId }).run();
  } catch {
...
function elapsedSec(row: any, now = Date.now()) {
  if (row?.state === 'playing' && Number(row.startedAt || 0) > 0) return Math.max(0, (now - Number(row.startedAt)) / 1000);
