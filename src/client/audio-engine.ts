...
type Graph = { ctx: AudioContext; master: GainNode; analyser: AnalyserNode; stops: AudioScheduledSourceNode[]; cleanups: Array<() => void>; startedAt: number; offsetSec: number; loopPattern: boolean; patternSec: number };

type BuildOptions = { live: boolean; durSec: number; fadeSec?: number; sampleRate?: number; loopPattern?: boolean; offsetSec?: number; delaySec?: number };
...
export function createAudioEngine(getSession: () => EntrainSessionV1) {
  ...
  function scheduleParam(param: AudioParam, pts: Keyframe[], key: 'beatHz'|'gainPct', map:(x:number)=>number, start:number, durSec:number, offsetSec = 0) {
  ...
  function buildLayer(ctx: BaseAudioContext, l: EntrainLayerV1, start:number, durSec:number, count:number, offsetSec = 0, live = false, cleanups: Array<() => void> = []) {
    const layerGain = ctx.createGain();
    scheduleParam(layerGain.gain, l.keyframes, 'gainPct', v => (v/100) * (0.55/Math.sqrt(Math.max(1,count))), start, durSec, offsetSec);
    ...
    if (l.type === 'sample') {
      ...
      scheduleSample(ctx, l, b, input, start, stopAt, stops, offsetSec, live, cleanups);
    ...
    if (l.type === 'procedural-ambience') {
      const src = proceduralAmbience(ctx, l.ambienceRecipe || 'pink-rain', l.seed || 1337);
      src.connect(input); src.start(start); src.stop(stopAt); stops.push(src); return { node: layerGain, stops };
    }
    if (l.type === 'carrier') {
    ...
    if (l.type === 'binaural' || l.type === 'monaural') {
      ...
      scheduleParam(a.frequency,l.keyframes,'beatHz', hz=>Math.max(20,carrier-hz/2), start, durSec, offsetSec);
      scheduleParam(b.frequency,l.keyframes,'beatHz', hz=>Math.max(20,carrier+hz/2), start, durSec, offsetSec);
    ...
    scheduleParam(lfo.frequency,l.keyframes,'beatHz',hz=>Math.max(.1,hz), start, durSec, offsetSec);
  ...
  function scheduleSample(ctx: BaseAudioContext, l: EntrainLayerV1, buffer: AudioBuffer, input: AudioNode, start: number, stopAt: number, stops: AudioScheduledSourceNode[], offsetSec = 0, live = false, cleanups: Array<() => void> = []) {
    ...
    if (loop.mode !== 'crossfade' || xfade < 0.02) {
      ...
      src.connect(input); src.start(start, phase); src.stop(stopAt); stops.push(src); return;
    }
    ...
    const scheduleOne = (t: number) => {
      ...
      const dur = Math.min(segment - Math.max(0, start - t), stopAt - visibleStart);
      if (dur <= 0.02) return;
    ...
    if (!live) { scheduleUntil(stopAt); return; }
  ...
  function build(ctx: BaseAudioContext, dest: AudioNode, opts: BuildOptions) {
    ...
    const session=getSession(), start=ctx.currentTime + delay, dur=opts.durSec || session.durationMin*60;
    ...
    if (shouldRepeat) {
      ...
      while (remaining > 0.02 && guard++ < 10000) {
        ...
        for (const l of layers) {
          const out=buildLayer(ctx,l,cycleStart,cycleDur,layers.length || 1, phase, !!opts.live, cleanups);
      ...
      for (const l of layers) {
        const out=buildLayer(ctx,l,start,dur,layers.length || 1, phase, !!opts.live, cleanups);
        out.node.connect(master); stops.push(...out.stops);
      }
    }
    ...
    if (opts.live) {
      const analyser=(ctx as AudioContext).createAnalyser(); analyser.fftSize=2048; comp.connect(analyser); analyser.connect(dest); return { master, analyser, stops, cleanups, startedAt:start, offsetSec: offset, loopPattern: !!opts.loopPattern, patternSec };
    }
    comp.connect(dest); return { master, analyser:null, stops, cleanups, startedAt:start, offsetSec: offset, loopPattern: !!opts.loopPattern, patternSec };
  }

  return {
    get running(){ return !!graph; },
    samples,
    async start(opts?: { loopPattern?: boolean; offsetSec?: number; delaySec?: number }){ ctx = ctx || new AudioContext(); await ctx.resume(); const session=getSession(); const liveSec = opts?.loopPattern ? Math.max(session.durationMin*60, 8*60*60) : Math.max(1, session.durationMin*60 - Math.max(0, opts?.offsetSec || 0)); const built = build(ctx,ctx.destination,{ live:true, durSec:liveSec, fadeSec:session.export?.fadeSec || 0, loopPattern: !!opts?.loopPattern, offsetSec: opts?.offsetSec || 0, delaySec: opts?.delaySec || 0 }); graph={ ctx, master:built.master, analyser:built.analyser!, stops:built.stops, cleanups:built.cleanups, startedAt:built.startedAt, offsetSec:built.offsetSec, loopPattern:built.loopPattern, patternSec:built.patternSec }; },
    stop(){ if(!graph) return; const current=graph; const t=current.ctx.currentTime; current.master.gain.setTargetAtTime(0.0001,t,.04); for (const fn of current.cleanups) { try{fn()}catch{} } setTimeout(()=>current.stops.forEach(s=>{try{s.stop()}catch{}}),180); graph=null; },
    rebuild(){ const was=!!graph; const offset=this.positionSec(); this.stop(); if(was) setTimeout(()=>this.start({ loopPattern:true, offsetSec:offset }),120); },
    positionSec(){ if(!graph) return 0; const raw = graph.offsetSec + Math.max(0, graph.ctx.currentTime - graph.startedAt); return graph.loopPattern ? raw : Math.min(raw, graph.patternSec); },
...
function proceduralAmbience(ctx: BaseAudioContext, recipe:string, seed:number) {
  const len=Math.floor(ctx.sampleRate*4), buf=ctx.createBuffer(2,len,ctx.sampleRate);
  const rnd = lcg(seed);
  for(let ch=0;ch<2;ch++){
    ...
    for(let i=0;i<len;i++){
      ...
      const w=rnd()*2-1;
      if(recipe==='brown-room') { last=(last+.018*w)/1.018; d[i]=last*2.8 + Math.sin(2*Math.PI*55*t+phaseOffset)*0.02; }
      else if(recipe==='bowl-drone') {
        const slow=(Math.sin(2*Math.PI*0.07*t+phaseOffset)*0.5+0.5);
        d[i]=(Math.sin(2*Math.PI*136.1*t+phaseOffset)*0.11 + Math.sin(2*Math.PI*204.2*t)*0.055 + Math.sin(2*Math.PI*272.4*t+phaseOffset)*0.035) * (0.72+0.28*slow) + w*0.012;
      } else {
        ...
        const pink=(b0+b1+b2+b3+b4+b5+b6+w*.5362)*.11; b6=w*.115926;
        const drops = recipe==='rain' && rnd() > .996 ? (rnd()*2-1)*0.45 : 0;
        d[i]=pink*0.8+drops;
      }
    }
  }
