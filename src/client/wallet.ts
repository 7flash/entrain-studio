...
export type TokenMeta = { ticker: string; displayName: string; chainId: string; tokenAddress: string };

let tokenMetaPromise: Promise<TokenMeta> | null = null;
let tokenMetaCache: TokenMeta = { ticker: 'WAVES', displayName: '$WAVES', chainId: 'solana', tokenAddress: '' };
...
export async function getTokenMeta(): Promise<TokenMeta> {
  if (!tokenMetaPromise) {
    tokenMetaPromise = fetch('/api/token/config')
      .then((x) => x.json())
      .then((r) => {
        tokenMetaCache = { ticker: r.ticker || 'WAVES', displayName: r.displayName || '$WAVES', chainId: r.chainId || 'solana', tokenAddress: r.tokenAddress || '' };
...
export async function paySol(recipient: string, lamports: number) {
  ...
  try { await connection.confirmTransaction({ signature, blockhash, lastValidBlockHeight }, 'confirmed'); } catch {}
