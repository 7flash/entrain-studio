import { sanitizeSession, type EntrainSessionV1, type EntrainLayerV1, type Keyframe } from './entrain-format';
...
export function signalMapForLayer(layer: EntrainLayerV1): LayerSignalMap {
  const keyframes = [...(layer.keyframes || [])].sort((a, b) => a.tMin - b.tMin);
  if (layer.type === 'binaural') {
    ...
    return {
      ...
      panNote: 'Pan intentionally disabled; binaural separation must remain hard L/R.',
  ...
  if (layer.type === 'carrier') {
    ...
    return {
      ...
      requiresHeadphones: false,
      panNote: panNote(layer),
      points: keyframes.map((k) => ({ tMin: round(k.tMin), gainPct: round(k.gainPct), gainDb: gainPctToDb(k.gainPct), carrierHz: carrier })),
    };
  }
  if (layer.type === 'noise') {
    return {
      ...
      formula: 'procedural noise bed, identical left/right before optional pan path',
      requiresHeadphones: false,
      panNote: panNote(layer),
...
function bedPoint(k: Keyframe): SignalMapPoint {
  return { tMin: round(k.tMin), gainPct: round(k.gainPct), gainDb: gainPctToDb(k.gainPct) };
}
...
export function fmtDb(db: number) {
  return db === -Infinity ? '-∞ dB' : `${db.toFixed(Math.abs(db) < 10 ? 1 : 0)} dB`;
}
...
export function formatSignalPoint(p: SignalMapPoint) {
  const core = p.leftHz != null && p.rightHz != null
...
function panNote(layer: EntrainLayerV1) {
  ...
  const motion = layer.panMotion?.rateHz ? ` · pan motion ${layer.panMotion.rateHz} Hz × ${layer.panMotion.depth}` : '';
  if (!pan && !motion) return undefined;
  const staticLabel = pan < 0 ? `${Math.abs(pan).toFixed(2)}L` : pan > 0 ? `${pan.toFixed(2)}R` : 'center';
